...
# ────────────────────────────────────────────────
# Kinetic Extractor with BROAD patterns
# ────────────────────────────────────────────────
class KineticExtractor:
    def __init__(self):
        self.patterns = {
            "missiles": re.compile(
                r'(?i)(?:fired|launched|detonated|barrage of|sent|targeted|reported|estimated|at least|more than|over|hundreds?|dozens?|salvo of)\s*(\d{1,6})\s*(?:ballistic|cruise|hypersonic|surface-to-surface)?\s*missiles?',
                re.IGNORECASE | re.DOTALL),

            "drones": re.compile(
                r'(?i)(?:launched|sent|fired|deployed|swarm of|hundreds?|dozens?|wave of)\s*(\d{1,6})\s*(?:attack|kamikaze|suicide|shahed|one-way|explosive)?\s*drones?',
                re.IGNORECASE | re.DOTALL),
            ...
            "casualties": re.compile(
                ...
                r'(?:at least|more than|over|nearly|around|about|approximately)?\s*'
                r'(\d{1,6})(?:\s*(?:people|civilians|soldiers|militants|personnel|troops|women|children|individuals)?)?'
                r'(?:\s*(?:and|plus|including)?\s*(?:at least|more than|over)?\s*(\d{1,6})\s*(?:wounded|injured|hurt))?)',
                re.IGNORECASE | re.DOTALL),

            "intercepted": re.compile(
                r'(?i)(?:intercepted|shot down|destroyed|neutralized|downed|engaged|taken out|air defenses downed|defenses brought down)\s*(\d{1,6})\s*(?:incoming\s*)?(?:ballistic|cruise|missiles?|drones?|projectiles?|incoming threats?)',
                re.IGNORECASE | re.DOTALL)
        }
...
with right_col:
    ...
    actors = {
        "Russia": "russia", "China": "china", "Turkey": "turkey", "Saudi Arabia": "saudi",
...
for _, row in df.sort_values("datetime", ascending=False).head(12).iterrows():
    ...
    tag_str = ' • '.join(tags) if tags else 'No kinetic data extracted'
    st.markdown(f"""
    <div style="background:#161b22; border:1px solid #30363d; border-radius:8px; padding:14px; margin-bottom:12px;">
        ...
        <span style="color:#ff7b72; font-size:0.92rem; font-weight:600;">{tag_str}</span>
    </div>
    """, unsafe_allow_html=True)

st.caption("📌 All figures are highest reported values per day (media consensus) • Subject to frequent revision • Not official military statistics • Sources: Reuters, AP, BBC, Al Jazeera, Times of Israel, Haaretz, Jerusalem Post, CNN, Google News")
